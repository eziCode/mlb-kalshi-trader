...
Download the default rolling window::
    ...
    .venv/bin/python data/download_scripts/download_live_kalshi_market_logs.py

Smoke-test two games without reusing cached trades::

    .venv/bin/python data/download_scripts/download_live_kalshi_market_logs.py --max-games 2 --refresh-trades --verbose

Download a fixed, reproducible game-date window (both dates inclusive)::

    .venv/bin/python data/download_scripts/download_live_kalshi_market_logs.py --start-date 2026-05-15 --end-date 2026-07-12
"""
...
def fetch_event_markets(
    ...
    *,
    ...
    for path in ("/historical/markets", "/markets"):
        # Kalshi's historical endpoint rejects mve_filter when event_ticker is
        # present, although the live endpoint accepts that combination.
